...
class Notification(models.Model):
	fromUser = models.ForeignKey(User,related_name='from_user') #usuario que crea la notificacion
	toUser = models.ForeignKey(User,related_name='to_user') #usuario que recibe la notificacion
	infoNotification = models.CharField(max_length=500) #texto de la notificacion
	projectNotification = models.ForeignKey(Project,related_name='project_notification') # proyecto relacionado a la notificacion
	created = models.DateTimeField(auto_now_add=True)
# 	visto = models.BooleanField(default=False)
	isActive = models.IntegerField(default=0) # no visto=0 | visto la primera vez=1 | no visto=-1
	def __unicode__(self):
		return unicode( str(self.fromUser.username)+" "+str(self.infoNotification)+" "+str(self.projectNotification.slug) );
# 			return unicode("Favs "+ str(self.fromUser.username));
	

class UserFavsAndNotificarions (models.Model):
	user = models.ForeignKey(User)									# Usuario que recibe las notificaciones
	fav_projects=models.ManyToManyField(Project,related_name='fav_projects',blank=True) # proyectos que creo el usuario y marco para que aparezcan en la barra lateral
	fav_collabs=models.ManyToManyField(Project,related_name='fav_collabs',blank=True) # proyectos en los que colaboro el usuario y marco para que aparezcan en la barra lateral
	notifications=models.ManyToManyField(Notification,related_name='notifi',blank=True)
	def __unicode__(self):
		return unicode("Favs "+ str(self.user.username));
