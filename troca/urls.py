...
urlpatterns = patterns(
    ...
    url(r'^descubre/(?P<category>\w+)/(?P<subcategory>\w+)/$', Sub_category_view_filter.as_view(), name='detailsubcategory'),
		url(r'^busqueda/(?P<busqueda>[-_ \w]+)/$', Search_view.as_view(), name='search'),
    ...
    url(r'^colaboraciones/(?P<username>\w+)/(?P<tipo>\w+)/$', login_required(Trueques.as_view()), name='trueques'),
		url(r'^notificaciones/(?P<username>\w+)/$', login_required(NotificacionView.as_view()), name='notificacionesview'),
	...
	#Troca 3
	url(r'^proyecto-a-favoritos/(?P<username>\w+)/(?P<projectSlug>[-_\w]+)/$', login_required(views.agregar_proyecto_favoritos), name='agregar_proyecto_favoritos'),
	url(r'^proyecto-no-a-favoritos/(?P<username>\w+)/(?P<projectSlug>[-_\w]+)/$', login_required(views.eliminar_proyecto_favoritos), name='eliminar_proyecto_favoritos'),
