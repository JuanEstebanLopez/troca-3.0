...
from models import Project, Skills_categories, Skills, UserProfile, Collaboration, CambioContrasena, Comment, Respond, Notification, UserFavsAndNotificarions
...
# //// métodos troca 3
@login_required
def crear_notificacion(username, usernameFrom, projectSlug, description): 
	user= User.objects.get(username=username) # Usuario al que le llega la solicitud
	userFrom=User.objects.get(username=usernameFrom)  # Usuario que envía la solicitud
	project = Project.objects.get(slug=projectSlug); # proyecto de la solicitud
	favs=get_favs(user);	#objeto con la información de las notificacioes
	##crea y agrega una nueva notificación
	new_notification=Notification(fromUser=userFrom, toUser=user,infoNotification=description,project=project);
	new_notification.save();
	favs.notifications.add(new_notification);
	favs.save();
	#Envía un correo
	mensaje=userFrom.username+" "+description+" en el proyecto "+project.title;
	enviar_correo(user.username, description, mensaje);
	return favs;

@login_required
def agregar_proyecto_favoritos(response,username,projectSlug):
...
# 	favs=get_favs(user)
	...
	if project.user==user :
		favs.fav_projects.add(project)
		favs.save()
	else:
		favs.fav_collabs.add(project)
		favs.save()
	return HttpResponseRedirect("/")

@login_required
def eliminar_proyecto_favoritos(username,projectSlug):
	user= User.objects.get(username=username) 
	#favs=get_favs(user);	
	favs= UserFavsAndNotificarions.objects.get(user=user)
	project = Project.objects.get(slug=projectSlug);
	if project.user==user:
		favs.fav_projects.remove(project);
		favs.save();
	else:
		favs.fav_collabs.remove(project);
		favs.save();	
	return HttpResponseRedirect("/");
...
class NotificacionView(PaginationMixin,ListView):
		""" Vista de la página donde se muestran los trueques de un usuario """
		...
		def get_context_data(self, **kwargs):
			...
			user = User.objects.get(username=self.kwargs['username']) # usuario del perfil.
			notificaciones = Notification.objects.all().filter(toUser=user) # colaboraciones hechas por el usuario.
			
			context['notificaciones'] = notificaciones
			for noti in notificaciones:
				if noti.isActive < 2:
					noti.isActive=noti.isActive+1
					noti.save();
			return context
...
def Close_trueque(request, collaboration,type_true ):   
	...
	if type_true == "1": # se acepta el trueque
		...
		# Elimina la habilidad de la lista de habilidades necesarias
		autor_project.category.remove(collaborator_skill_to_autor)
		#	Cierra el proyecto si no necesita más habilidades
		if len(autor_project.category.all())== 0:
			autor_project.num_needs=100
			autor_project.save()
			# Agrega la habilidad a la lista de habilidades suplidas
		...
		if collaborator_project:
			...
			#	Cierra el proyecto si no necesita más habilidades
			if len(collaborator_project.category.all())== 0:
				collaborator_project.num_needs=100
				collaborator_project.save()
			# Agrega la habilidad a la lista de habilidades suplidas
...
class Home(ListView):
	...
	def get_context_data(self, **kwargs):
		...
		_selProjects = Project.objects.all().filter(isActive=True).filter(num_needs__lt = 100) [:6]
		context['selected'] = _selProjects
		
		collabs=Collaboration.objects.all().filter(isActive=1).order_by('-date')[:6];
		context['last_collabs'] = collabs;
		
		hab=Skills.objects.all();
		context['hab'] = hab;
	...
	def get_queryset(self):
		...
		return qs.filter(isActive=True).filter(num_needs__lt = 100).order_by('-date')[:6]
...
class Sub_category_view_filter(PaginationMixin, ListView):
	...
	template_name = 'detail_sub_category.html'
	model = Project
	paginate_by = 9
...
class Search_view(TemplateView):
	""" Vista de la página donde se muetran los resultados de la búsqueda """
	...
	def get_context_data(self, **kwargs):
		...
		qs = Project.objects.all().filter(isActive=True)
		skills = {}
		proyectos=set([])
		usuarios=set([])	
		...
		for x in qs:
			if is_proyecto_coincide(busqueda, x, 1): # título
				proyectos.add(x)	
		for x in qs:
			if  not x in proyectos:
				if is_proyecto_coincide(busqueda, x, 2): # habilidades
					proyectos.add(x)	
		for x in qs:
			if  not x in proyectos:
				if is_proyecto_coincide(busqueda, x, 3): # descripciones
					proyectos.add(x)	
		usus = UserProfile.objects.all();
		for u in usus:
			if is_usuario_coincide(busqueda,u,1):
				usuarios.add(u);
		for u in usus:
			if  not u in usuarios:
				if is_usuario_coincide(busqueda,u,2):
					usuarios.add(u);
...
def is_proyecto_coincide(busqueda,proyecto, tipo):
	b=slugify(busqueda);
	coincide=False;			
	if tipo==1: # en el titulo
		if b in slugify(proyecto.title):
			coincide=True;
	elif tipo==2:	# alguna hailidad
		c= proyecto.category.all();
		for cat in c:
			if b in slugify(cat.title):
				coincide=True;
		if not coincide:
			cs= proyecto.category_supp.all();
			for cat in cs:
				if b in slugify(cat.title):
					coincide=True;
	elif tipo==3: # en la descripción o contenido
		if b in slugify(proyecto.summary):
			coincide=True;
		elif b in slugify(proyecto.content):
			coincide=True;

	return coincide;
def is_usuario_coincide(busqueda, usuario, tipo):
	b=slugify(busqueda);
	coincide=False;	
	if tipo==1: # en el nombre
		if b in slugify(usuario.user.username):
			coincide=True;
		elif b in slugify(usuario.user.first_name):
			coincide=True;
		elif b in slugify(usuario.user.last_name):
			coincide=True;
	elif tipo==2:	# alguna hailidad
		c= usuario.category.all();
		for cat in c:
			if b in slugify(cat.title):
				coincide=True;
	return coincide;
...
def register_collaborator(request, username, slug):
	...
	text_content = 'Has recibido una nueva colaboración!'
	html_content = ('<p>El usuario <strong>' + str(loggedUser) + '</strong> quiere colaborar en tu proyecto <a href="http://troca.cc'+ s +'">' + '<strong>' + project_base.title + '</strong>' + '</a></p>')
	msg = EmailMultiAlternatives(subject, text_content, from_email, [to])
	msg.attach_alternative(html_content, "text/html")
	msg.send()
	return HttpResponseRedirect(s)
...
class Profile(PaginationMixin, DetailView):
	...
	def get_context_data(self, **kwargs):
		...
		collaborations = Collaboration.objects.all().filter(collaborator=_user).filter(isActive=1)
		rev_collaborations = Collaboration.objects.all().filter(autor=_user).filter(isActive=1)
		user_collaborations=[];
		for col in collaborations:
			user_collaborations.append(col.project)
		for col in rev_collaborations:
			if col.collaboratorProject:
				user_collaborations.append(col.collaboratorProject)
		
# 		sadasdasdasd
		context['hasProjects'] = user_projects.exists()
		context['projects'] = user_projects
		context['hasCollaborations'] = len(set(user_collaborations)) > 0
		context['collaborations'] = user_collaborations
